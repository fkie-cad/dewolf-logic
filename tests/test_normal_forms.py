...
class TestDNF:
    def test_z3_to_cnf_negation(self):
        ...
        w.define(var := w.variable("v", 8), w.bitwise_negate(w.variable("x", 1)))
        ToDnfVisitor(var)
        assert World.compare(w.get_definition(var), w.bitwise_negate(w.variable("x", 1)))

    def test_is_in_dnf(self):
        w = World()
        w.define(
            var := w.variable("v", 8),
            w.bitwise_or(w.bitwise_and(w.variable("x", 8), w.variable("a", 8)), w.bitwise_and(w.variable("x", 8), w.variable("b", 8))),
        )
        ToDnfVisitor(var)
        assert World.compare(
            w.get_definition(w.variable("v", 8)),
    ...
    def test_is_in_nested_dnf(self):
        w = World()
        w.define(
            var := w.variable("v", 8),
            w.bitwise_or(
                w.bitwise_or(w.variable("z", 8), w.bitwise_or(w.variable("y", 8), w.variable("u", 8))),
                w.bitwise_and(w.variable("x", 8), w.variable("a", 8)),
                w.bitwise_and(w.variable("x", 8), w.variable("b", 8)),
            ),
        )
        ToDnfVisitor(var)
        assert World.compare(
    ...
    def test_is_in_dnf_contains_negation(self):
        w = World()
        w.define(var := w.variable("v", 8), w.from_string("(| (& (~x1@8) x2@8) (&x3@8 (~x1@8)))"))
        ToDnfVisitor(var)
        assert World.compare(w.get_definition(w.variable("v", 8)), w.from_string("(| (& (~x1@8) x2@8) (&x3@8 (~x1@8)))"))

    def test_in_cnf(self):
        w = World()
        w.define(
            var := w.variable("v", 8),
            w.bitwise_and(w.bitwise_or(w.variable("x", 8), w.variable("y", 8)), w.bitwise_or(w.variable("u", 8), w.variable("z", 8))),
        )
        ToDnfVisitor(var)
        assert World.compare(
            w.get_definition(w.variable("v", 8)),
            w.bitwise_or(
                w.bitwise_and(w.variable("u", 8), w.variable("x", 8)),
                w.bitwise_and(w.variable("z", 8), w.variable("x", 8)),
                w.bitwise_and(w.variable("u", 8), w.variable("y", 8)),
                w.bitwise_and(w.variable("z", 8), w.variable("y", 8)),
            ),
        )
    ...
    def test_one_wrong_operand(self):
        ...
        assert World.compare(
            w.get_definition(w.variable("v", 8)),
            w.bitwise_or(
                w.bitwise_and(w.bitwise_negate(w.variable("x1", 8)), w.variable("x2", 8)),
                w.bitwise_and(w.variable("x3", 8), w.bitwise_negate(w.variable("x1", 8))),
                w.bitwise_and(w.variable("x2", 8), w.variable("x4", 8)),
                w.bitwise_and(w.variable("x3", 8), w.variable("x4", 8)),
            ),
        )

    def test_in_cnf_negation(self):
        w = World()
        w.define(var := w.variable("v", 8), w.from_string("(& (| x2@1 (~x1@1)) (| x3@1 (~x1@1)) )"))
        ToDnfVisitor(var)
        assert World.compare(w.get_definition(w.variable("v", 8)), w.from_string("(| (~x1@1) (& x3@1 x2@1) )"))
    ...
    def test_with_negation_of_operand(self):
        ...
        assert World.compare(
            w.get_definition(w.variable("v", 1)),
            w.bitwise_or(
                w.bitwise_and(w.variable("x5", 1), w.variable("x3", 1), w.variable("x2", 1), w.variable("x1", 1)),
                w.bitwise_and(w.variable("x4", 1), w.variable("x3", 1), w.variable("x2", 1), w.variable("x1", 1)),
            ),
        )
    ...
    def test_mix_formula(self):
        ...
        assert World.compare(
            w.get_definition(w.variable("v", 1)),
            w.bitwise_or(
                w.bitwise_and(w.variable("x2", 1), w.variable("x4", 1), w.variable("x3", 1)),
                w.bitwise_and(w.bitwise_negate(w.variable("x1", 1)), w.variable("x3", 1), w.variable("x4", 1)),
                w.bitwise_and(w.bitwise_negate(w.variable("x1", 1)), w.variable("x5", 1)),
            ),
        )
